...
module.exports = function generateBraceletPattern(colors, size) {
  ...
  if (new Set(colors).size !== colors.length) {
    throw new Error("'colors' must contain unique colors");
  }
  ...
  // There is a possibility of an infinite loop here, but it is very unlikely
  // This counter is here to prevent that from happening
  let counter = 0;
  ...
  while (bracelet.length < size) {
    ...
    if (usedPatterns.has(pattern)) {
      ...
      if (counter > 100) {
        throw new Error(
          "Unable to generate bracelet pattern. Try adjusting the number of colors and length of the bracelet."
        );
      }
      continue;
    }
